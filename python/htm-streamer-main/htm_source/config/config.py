...
def build_enc_params(features_cfg: dict,
    ...
    for f, f_dict in features_cfg.items():
        if f not in data_samples.columns:
            print(f"Ignoring feature: `{f}`")
            continue

        dtype = to_htm_type(f_dict['type'])
        ...
        if dtype is HTMType.Numeric:
            ...
            if res is None:
                print(f"No resolution supplied for `{f}` - one will be deduced. (Not recommended)")
                res = get_rdse_resolution(feature=f, f_sample=f_sample)
        ...
        # get enc - categorical
        elif dtype is HTMType.Categorical:
        ...
        # set size, activeBits
        features_enc_params[f]['type'] = dtype
        if isinstance(enc_size, (list, tuple)) and len(enc_size) > 1:
            ...
            flat_size = math.prod(enc_size)
            features_enc_params[f]['shape'] = tuple(enc_size)
            features_enc_params[f]['size'] = flat_size
            features_enc_params[f]['activeBits'] = round(encoders_cfg['w'] * flat_size)

        else:  # if list of 1 value
            if isinstance(enc_size, (list, tuple)):
                enc_size = enc_size[0]

            weighted_size = int(enc_size * features_weights[f])
            features_enc_params[f]['size'] = weighted_size
            features_enc_params[f]['activeBits'] = round(encoders_cfg['w'] * weighted_size)
