...
def get_random_sig(upper: int = 10_000) -> int:
...
def get_args():
    ...
    parser.add_argument('-c', '--config_path_data', required=True,
...
def smooth_predictions(predictions: np.ndarray, filter_size: int) -> np.ndarray:
    return np.correlate(predictions, np.ones(filter_size) / filter_size, mode='same')
...
@nb.jit(nopython=True, nogil=True, cache=True)
def get_slices(array: np.ndarray):
...
def get_layer_config(config: dict, layer: int) -> dict:
    layer_cfg = {}
    for key, value in config.items():
        if isinstance(value, (tuple, list)):
            layer_cfg[key] = value[layer]
        else:
            layer_cfg[key] = value
